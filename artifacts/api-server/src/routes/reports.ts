...
router.get("/monthly", (req, res) => {
  ...
  const revenueByDay = db.prepare("SELECT date, SUM(amount) as revenue, COUNT(*) as appointments FROM financial_transactions WHERE type='receita' AND date LIKE ? GROUP BY date ORDER BY date").all(`${prefix}%`) as any[];
...
router.get("/services", (req, res) => {
  ...
  const totalServices = (db.prepare(`SELECT COUNT(*) as c FROM appointments a ${where}`).get(...params) as any).c;
  const totalRevenue = (db.prepare(`SELECT COALESCE(SUM(final_price),0) as s FROM appointments a WHERE a.status='concluido' ${params.length ? "AND " + where.slice(9) : ""}`).get(...params) as any).s;

  const byCategory = db.prepare(`
    SELECT s.category, COUNT(*) as count, COALESCE(SUM(a.final_price),0) as revenue
    FROM appointments a JOIN services s ON s.id = a.service_id ${where}
    GROUP BY s.category ORDER BY count DESC
  `).all(...params) as any[];

  const byStatus = db.prepare(`
    SELECT status, COUNT(*) as count FROM appointments a ${where} GROUP BY status
  `).all(...params) as any[];
